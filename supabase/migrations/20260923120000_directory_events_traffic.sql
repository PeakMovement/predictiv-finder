...
--    session_id stays the long lived visitor id so existing rows keep meaning
--    what they meant. visit_id resets when the tab closes.
ALTER TABLE public.directory_events
...
CREATE INDEX IF NOT EXISTS directory_events_day_idx
  ON public.directory_events ((created_at::date));

-- 3. Reporting views.
--    security_invoker keeps the caller's RLS in force, so these views are
--    readable only by a blog admin, exactly like the table underneath.
...
-- Searches that came back with nothing. This is the recruitment queue: it says
-- which profession and suburb combinations people want and Predictiv cannot
-- serve yet.
...
-- The commercial report. What to put in front of a practice when asking them
-- to approve their listing.
