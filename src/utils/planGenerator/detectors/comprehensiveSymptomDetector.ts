...
export function detectComprehensiveSymptoms(userInput: string): {
  ...
  try {
    ...
    // Determine which symptoms are likely primary vs secondary based on:
    // - Word proximity to key phrases like "mainly", "primarily", etc.
    // - Repetition of the symptom or related terms throughout the input
    // - Position in the text (symptoms mentioned early often have higher priority)
