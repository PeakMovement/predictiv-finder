import { Practitioner } from '@/types';

export const STRENGTH_COACHES: Practitioner[] = [
  {
    id: "sc1",
    name: "Sipho Ngcobo",
    serviceType: "personal-trainer",
    pricePerSession: 300,
    maxPrice: 900,
    serviceTags: ["functional training", "group classes", "strength building"],
    location: "Wynberg, Cape Town",
    isOnline: false,
    availability: "Mon-Fri, flexible hours",
    imageUrl: "/placeholder.svg",
    bio: "Functional training specialist focusing on group gym classes. Experienced in developing comprehensive strength programs.",
    rating: 4.8,
    specialtyNotes: "Group gym classes, functional training"
  },
  {
    id: "sc2",
    name: "Anke du Plessis",
    serviceType: "personal-trainer",
    pricePerSession: 350,
    maxPrice: 1000,
    serviceTags: ["powerlifting", "personalized plans", "strength training"],
    location: "Vredehoek, Cape Town",
    isOnline: true,
    availability: "Early mornings and evenings",
    imageUrl: "/placeholder.svg",
    bio: "Powerlifting specialist offering personalized gym plans. Focuses on proper form and progressive overload.",
    rating: 4.9,
    specialtyNotes: "Powerlifting, personalized gym plans"
  },
  {
    id: "sc3",
    name: "Zanele Khumalo",
    serviceType: "personal-trainer",
    pricePerSession: 300,
    maxPrice: 800,
    serviceTags: ["women's strength", "bodyweight training", "functional fitness"],
    location: "Mowbray, Cape Town",
    isOnline: true,
    availability: "Flexible hours",
    imageUrl: "/placeholder.svg",
    bio: "Specializes in women's strength training with a focus on bodyweight exercises. Creates inclusive and supportive training environments.",
    rating: 4.7,
    specialtyNotes: "Women's strength training, bodyweight focus"
  },
  {
    id: "sc4",
    name: "Maria Santos",
    serviceType: "personal-trainer",
    pricePerSession: 280,
    maxPrice: 850,
    serviceTags: ["olympic lifting", "crossfit", "athletic performance"],
    location: "Salt River, Cape Town",
    isOnline: false,
    availability: "Flexible hours",
    imageUrl: "/placeholder.svg",
    bio: "Olympic lifting coach specializing in CrossFit and athletic performance enhancement.",
    rating: 4.8,
    specialtyNotes: "Olympic lifting, CrossFit"
  },
  {
    id: "sc5",
    name: "John Peterson",
    serviceType: "personal-trainer",
    pricePerSession: 320,
    maxPrice: 950,
    serviceTags: ["senior fitness", "rehabilitation", "functional training"],
    location: "Pinelands, Cape Town",
    isOnline: true,
    availability: "Morning sessions",
    imageUrl: "/placeholder.svg",
    bio: "Specialized in senior fitness and post-rehabilitation strength training.",
    rating: 4.7,
    specialtyNotes: "Senior fitness specialist"
  },
  {
    id: "sc6",
    name: "Nomvula Dlamini",
    serviceType: "personal-trainer",
    pricePerSession: 290,
    maxPrice: 870,
    serviceTags: ["bodyweight training", "calisthenics", "mobility"],
    location: "Observatory, Cape Town",
    isOnline: true,
    availability: "Evening sessions",
    imageUrl: "/placeholder.svg",
    bio: "Calisthenics expert focusing on bodyweight strength and mobility improvement.",
    rating: 4.9,
    specialtyNotes: "Calisthenics and mobility"
  },
  {
    id: "sc7",
    name: "Marcus Williams",
    serviceType: "personal-trainer",
    pricePerSession: 310,
    maxPrice: 900,
    serviceTags: ["powerlifting", "strongman training", "strength development"],
    location: "Century City, Cape Town",
    isOnline: false,
    availability: "Morning sessions",
    imageUrl: "/placeholder.svg",
    bio: "Powerlifting and strongman specialist focusing on strength development.",
    rating: 4.8,
    specialtyNotes: "Powerlifting and strongman"
  },
  {
    id: "sc8",
    name: "Lucy Chen",
    serviceType: "personal-trainer",
    pricePerSession: 280,
    maxPrice: 850,
    serviceTags: ["functional training", "crossfit", "group classes"],
    location: "Gardens, Cape Town",
    isOnline: true,
    availability: "Flexible hours",
    imageUrl: "/placeholder.svg",
    bio: "CrossFit coach specializing in functional training and group classes.",
    rating: 4.7,
    specialtyNotes: "CrossFit and functional training"
  },
  {
    id: "sc9",
    name: "Trevor Ndlovu",
    serviceType: "personal-trainer",
    pricePerSession: 300,
    maxPrice: 880,
    serviceTags: ["strength building", "injury prevention", "form correction"],
    location: "Constantia, Cape Town",
    isOnline: false,
    availability: "Evening sessions",
    imageUrl: "/placeholder.svg",
    bio: "Form specialist focusing on injury prevention and proper technique.",
    rating: 4.8,
    specialtyNotes: "Form and injury prevention"
  },
  {
    id: "sc10",
    name: "Amanda du Toit",
    serviceType: "personal-trainer",
    pricePerSession: 290,
    maxPrice: 870,
    serviceTags: ["strength endurance", "circuit training", "group fitness"],
    location: "Sea Point, Cape Town",
    isOnline: true,
    availability: "Morning and evening",
    imageUrl: "/placeholder.svg",
    bio: "Circuit training specialist with focus on strength endurance.",
    rating: 4.7,
    specialtyNotes: "Circuit and endurance training"
  },
  {
    id: "sc11",
    name: "James Peterson",
    serviceType: "personal-trainer",
    pricePerSession: 340,
    maxPrice: 950,
    serviceTags: ["powerlifting", "strength building", "competition prep"],
    location: "Sea Point, Cape Town",
    isOnline: false,
    availability: "Morning sessions",
    imageUrl: "/placeholder.svg",
    bio: "Competition powerlifting coach with focus on strength development.",
    rating: 4.8,
    specialtyNotes: "Powerlifting specialist"
  },
  {
    id: "sc12",
    name: "Nomvula Dube",
    serviceType: "personal-trainer",
    pricePerSession: 300,
    maxPrice: 880,
    serviceTags: ["functional fitness", "mobility", "group training"],
    location: "Woodstock, Cape Town",
    isOnline: true,
    availability: "Flexible hours",
    imageUrl: "/placeholder.svg",
    bio: "Functional fitness expert specializing in mobility and group sessions.",
    rating: 4.7,
    specialtyNotes: "Functional fitness expert"
  },
  {
    id: "sc13",
    name: "Ryan Cooper",
    serviceType: "personal-trainer",
    pricePerSession: 320,
    maxPrice: 900,
    serviceTags: ["olympic lifting", "athletic performance", "strength"],
    location: "Green Point, Cape Town",
    isOnline: false,
    availability: "Afternoon sessions",
    imageUrl: "/placeholder.svg",
    bio: "Olympic weightlifting coach focused on athletic performance.",
    rating: 4.9,
    specialtyNotes: "Olympic lifting coach"
  },
  {
    id: "sc14",
    name: "Sarah van Wyk",
    serviceType: "personal-trainer",
    pricePerSession: 290,
    maxPrice: 850,
    serviceTags: ["women's strength", "postpartum fitness", "body transformation"],
    location: "Claremont, Cape Town",
    isOnline: true,
    availability: "Morning and evening",
    imageUrl: "/placeholder.svg",
    bio: "Women's strength specialist with focus on postpartum fitness.",
    rating: 4.8,
    specialtyNotes: "Women's strength specialist"
  },
  {
    id: "sc15",
    name: "David Mbatha",
    serviceType: "personal-trainer",
    pricePerSession: 310,
    maxPrice: 890,
    serviceTags: ["calisthenics", "bodyweight training", "mobility"],
    location: "Observatory, Cape Town",
    isOnline: true,
    availability: "Flexible hours",
    imageUrl: "/placeholder.svg",
    bio: "Calisthenics expert specializing in bodyweight strength training.",
    rating: 4.7,
    specialtyNotes: "Calisthenics specialist"
  }
];
